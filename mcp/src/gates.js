...
const scopeMatch = {
    ...
    check(ctx) {
        ...
        if (!affectedMatch)
            return []; // No Affected Files section → skip
        ...
        for (const file of ctx.diff.files) {
            const isDeclared = declared.some((d) => file === d || file.startsWith(d.replace(/\/$/, "")));
            if (!isDeclared) {
                violations.push({
                    ruleId: "scope-match",
                    severity: "MEDIUM",
...
const checklistComplete = {
    ...
    check(ctx) {
        const incomplete = ctx.checklist.filter((item) => !item.done && !item.desc.includes("~~"));
    ...
    check(ctx) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!line.startsWith("+") || line.startsWith("++"))
                continue; // Skip non-additions and file headers
...
const reproducibility = {
    ...
    check(ctx) {
        const violations = [];
        const rFiles = ctx.diff.files.filter((f) => f.endsWith(".R") || f.endsWith(".qmd"));
        ...
        for (const file of pyFiles) {
            const filePatch = ctx.diff.patch;
            const hasRandom = /np\.random\.|random\.random|random\.randint|random\.choice/i.test(filePatch);
            const hasSeed = /np\.random\.seed|random\.seed|PYTHONHASHSEED/i.test(filePatch);
            if (hasRandom && !hasSeed) {
                violations.push({
                    ruleId: "reproducibility",
                    severity: "MEDIUM",
...
const convergenceCheck = {
    ...
    check(ctx) {
        ...
        const outputFiles = ctx.diff.files.filter((f) => f.endsWith(".log") ||
            f.endsWith(".out") ||
            f.endsWith(".txt") ||
            f.includes("output"));
...
const scopeCreep = {
    ...
    check(ctx) {
        ...
        for (const item of outOfScopeItems) {
            ...
            const matching = ctx.diff.files.filter((f) => f.toLowerCase().includes(item.toLowerCase()) || item.toLowerCase().includes(f.toLowerCase()));
        ...
        if (hasScope && !hasSpecBoundary) {
            ...
            if (affectedMatch && scopeMatch2) {
                const declared = affectedMatch[1]
                    ...
                    .map((l) => l.replace(/^-\s*`?/, "").replace(/`?\s*$/, "").trim())
                    .filter(Boolean);
                const undeclared = ctx.diff.files.filter((f) => !declared.some((d) => f === d || f.startsWith(d.replace(/\/$/, ""))));
...
const temporalMarking = {
    ...
    check(ctx) {
        ...
        const hasTemporalMark = ctx.commits.some((c) => /ex post|post-hoc|exploratory|retroactive/i.test(c));
...
// ── Runner ──
function runAllGates(ctx, mode = "pr") {
    const violations = [];

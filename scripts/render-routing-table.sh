...
TABLE_PATH="$REPO_DIR/.fu-routing.json"
LEGACY_TABLE_PATH="$REPO_DIR/.academic-git-routing.json"
...
if [ ! -f "$TABLE_PATH" ] && [ -f "$LEGACY_TABLE_PATH" ]; then
  TABLE_PATH="$LEGACY_TABLE_PATH"
fi

...
def main() -> int:
    ...
    if connector_created and not (used_issue_start or (used_develop and used_worktree_add)):
        return block(
            ...
            "requires `handle-issue` to route through `start_issue` or the "
            "`/codex-gh-issue-start` workflow instead. Because the issue already exists, "
            "continue by using `gh issue develop <issue-number-or-url> --name "
            ...
            "`start_issue` or `/codex-gh-issue-start` instead of the GitHub connector."
        )
    ...
    if raw_issue_create and not (used_develop and used_worktree_add):
        return block(
            ...
            "`handle-issue` to route through `start_issue` or `/codex-gh-issue-start`, "
            "which create the issue-linked branch and open it in a dedicated git worktree. "
            "Continue by creating that linked worktree now, or redo future issue creation "
            "through the canonical issue-start path."
        )

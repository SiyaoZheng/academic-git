...
STATE_FILE="$(git rev-parse --git-path scholaros-posttool-last-head 2>/dev/null || echo "")"
...
SYSTEM_MESSAGE="[ScholarOS] Detected new commit ${SHORT_SHA} on '${BRANCH}': ${SUBJECT}"
ADDITIONAL_CONTEXT="A new git commit was created in this ScholarOS repository after a Bash tool call. Current branch: ${BRANCH}. New HEAD: ${SHORT_SHA}. Subject: ${SUBJECT}."

if [ "$DIRECT_COMMIT" = true ]; then
  SYSTEM_MESSAGE="${SYSTEM_MESSAGE}. Direct shell commit flow bypasses ScholarOS commit checks."
